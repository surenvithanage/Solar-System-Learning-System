...
export class EndpointManagementComponent implements OnInit {
  ...
  public model = new EndpointModel('', '', '', '');
  public updateModel = new EndpointModel('', '', '', '');
  ...
  loadList() {
    this.service.loadEndpointList().subscribe(
      data => {
  ...
  onSubmit() {
    this.service.createEndpoint(this.model).subscribe(
      data => {
  ...
  onUpdate() {
    ...
    this.service.updateEndpoint(this.updateModel).subscribe(
      data => {
  ...
  delete(payload) {
    this.service.deleteEndpoint(payload).subscribe(
      data => {
